...
@section('content')
    ...
    <div class="w-min-max mt-24 mx-auto">
      <h1 class="text-center font-semibold s text-lg">Editar Funcionarios</h1>
    </div>
    ...
    <form action="/funcionarios/update/{{$funcionario->id}}" method="POST">
      ...
      @method('PUT')
        <div class="mt-10 w-1/3 rounded-lg shadow-md mx-auto flex items-center justify-center flex-col bg-white">
            ...
            <input type="text" name="nome" value="{{$funcionario->nome}}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div class="mt-3">
            <label for="email" class="block text-gray-700 text-sm font-bold mb-2">Email</label>
            <input type="email"  name="email" placeholder="Ex: johndoe@hotmail.com" value="{{$funcionario->email}}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div class="mt-3">
            <label for="cpf" class="block text-gray-700 text-sm font-bold mb-2">CPF</label>
            <input type="text" name="cpf" pattern="(\d{3}\.?\d{3}\.?\d{3}-?\d{2})|(\d{2}\.?\d{3}\.?\d{3}/?\d{4}-?\d{2})" value="{{$funcionario->cpf}}" placeholder="Ex: 54332146122" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div class="mt-3">
            <label for="numeroCelular" class="block text-gray-700 text-sm font-bold mb-2">Numero de Celular</label>
            <input type="text" placeholder="Digite os numeros tudo junto" pattern="[0-9]{9}" value={{$funcionario->numeroCelular}} name="numeroCelular"class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            <div class="mt-3">
            <label for="dataNascimento" class="block text-gray-700 text-sm font-bold mb-2">Data de Nascimento</label>
            <input type="date" name="dataNascimento" value="{{$funcionario->dataNascimento}}" placeholder="Ex: 20/01/2002" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mr-12 leading-tight focus:outline-none focus:shadow-outline">
            </div>
            ...
            <label for="setor" class="block float-center ml-2 text-gray-700 text-sm font-bold mb-2">Selecione o Setor:</label>
                ...
                @foreach ($setorOption as $options )
                    <option value="{{$funcionario->setor}}">{{$options}}</option>
                @endforeach
                </select>
            </div>
            <input type="submit" value="Finalizar e Editar" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-8 mt-4 mb-4 rounded" >
        </div>
